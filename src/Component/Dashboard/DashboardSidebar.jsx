      ...
      `}>
        <div className="mb-0">
          <div className="flex items-center gap-3 font-bold text-xl px-2 py-6">
            <div className={`p-2 rounded-xl shadow-md rotate-3 group-hover:rotate-0 transition-transform ${
        ...
        <nav className="flex-1 mt-4 space-y-2">
          {menuItems.map(item => (
            ...
            >
              <span className={`${activeSection === item.id ? 'text-white' : isDarkMode ? 'text-blue-400 group-hover:scale-110' : 'text-blue-600 group-hover:scale-110'} transition-transform`}>
                {item.icon}
              </span>
        ...
        <div className={`mt-auto pt-4 border-t ${isDarkMode ? 'border-gray-700' : 'border-gray-100'}`}>
          ...
          >
            <LogOut size={20} className={`${isDarkMode ? 'text-gray-500 group-hover:text-red-400' : 'text-gray-400 group-hover:text-red-600'} transition-colors`} />
